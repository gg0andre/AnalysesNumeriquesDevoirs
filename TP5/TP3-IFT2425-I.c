//------------------------------------------------------
// module  : Tp5-IFT2425-I.c - Integration & Erreurs Numeriques, Suite et Chaos (Devoir3)
// questions : 1 - 2(a) - 2(b)
...
static inline float fonction (float x) {
	return 4.0 * sqrt(1 - x*x);
}

//Fonction pour calculer par la methode du trapeze
//on va calculer f(x) = h[(f(0)+f(n))/2 + f(x1) + f(x2) + ....]
float trapeze(int n, float Pi) {
  int i;
  float f0 = fonction(0);   //x0 = 0
  float fn = fonction(1);   //xn = 1
  float fx, resultat;

  resultat = (f0+fn)/2;
  for (i=1; i<n; i++) {
    fx = fonction(i / (float) n);   //xi = 1/n
    resultat += fx;
  } 

  resultat/=(float) (n+1);
  float erreur = fabs(resultat - Pi);
  float logEr = log10(erreur);

  printf("\nMethode du trapeze : Pi=%0.10f Er=%0.10f LogEr=%0.10f\n", resultat, erreur, logEr);
  return resultat;
}


//Fonction de la question 2a
void vecteurFlottante(int n, float* vecteur) {
  float fx;
  float nf = (float) n;
  int i;

  vecteur[0] = fonction(0) * 1/2 * 1/nf;

  for (i = 1; i < n-1; i++)  {
    fx = fonction(i/nf);
    vecteur[i] = fx/nf;
  }
  vecteur[n-1] = fonction(i/nf) * 1/2 * 1/nf;
}

//fonction pour calculer la somme par l'algo DPR de la question 2a
float sommeDPR(float* vecteur, int a, int b) {
 if (a - b == 0)  {
   return vecteur[a];
 } else if (a - b == 1) {
   return vecteur[a] + vecteur[b];
 } else {
   int moitie = a + (b-a)/2;
   return sommeDPR(vecteur, a, moitie) + sommeDPR(vecteur, moitie+1, b);
 } 
}

//fonction pour appeller sommeDPR
float appelSommeDPR(float* vecteur, int n, float PI) {
   float resultat = sommeDPR(vecteur, 0, n);
   float erreur = fabs(resultat - PI);
   float logEr = log10(erreur);

   printf("Somme pour algo DPR : Pi=%0.10f Er=%0.10f LogEr=%0.10f\n", resultat, erreur, logEr); 
   return resultat;
}

//fonction pour calculer la somme de Kahan
float sommeKahan(float* vecteur, int n, float PI) {
  float e, s, temp, y;
  e = s = temp = y = 0;
  int i;
  ...
  printf("Somme de Kahan : Pi=%0.10f Er=%0.10f LogEr=%0.10f\n", s, erreur, logEr); 
  return s;
}
//----------------------------------------------------------
//----------------------------------------------------------
...
//--------------------------------------------------------------------------------
 ...
 //Programmer ici
 float PI1 = trapeze(NBINTERV, PI);

 float* vecteur = VctPts;
 vecteurFlottante(NBINTERV + 1, vecteur);
 appelSommeDPR(vecteur, NBINTERV+1, PI);
 

 float s = sommeKahan(vecteur, NBINTERV+1, PI);
