...
position RungeKuttaFehlberg(position pos, vitesse vit) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    double tempV[3] = {X_1, X_2, X_3};   //tableau temporaire
    for (j=0; j<3; j++) {
...
int converger(double x, double y, double* vecteurX, double* vecteurY) {
  int i=0;
  double dist = distance(x, vecteurX[0], y, vecteurY[0]);
  double smallD = dist;
  double smallDIndex = i;
  for (i=1; i < 3; i++) {
    dist = distance(x, vecteurX[i], y, vecteurY[i]);
    if (dist <= smallD) {
      smallD = dist;
      smallDIndex = i;
    }
...
int main (int argc, char **argv)
{
  ...
  for (i = 0; i <HEIGHT; i++) {
    for (j = 0; j < WIDTH; j++) {
      position.x = (j / (double) HEIGHT) * MAX_X - MAX_X/2.0;
      position.y = (1.0 - i/(double) WIDTH) * MAX_Y - MAX_Y/2.0;

      nAimantI = converger(position.x, position.y, vecteurX, vecteurY);
      nAimantD = distance(position.x, vecteurX[nAimantI], position.y, vecteurY[nAimantI]);
      proche = nAimantD < 0.5;

      for (k = 1; k < (int) NB_INTERV; k++) {
        position = RungeKuttaFehlberg(position, vitesse);

        vitesse.x += H * acceleration(vitesse.x, position.x, position.y, vecteurX, vecteurY);
        vitesse.y += H * acceleration(vitesse.y, position.y, position.x, vecteurY, vecteurX);

        cAimantI = converger(position.x, position.y, vecteurX, vecteurY);

        if(cAimantI != nAimantI) {
          nAimantI = cAimantI;
          converge = 0;
        }

        nAimantD = distance(position.x, vecteurX[cAimantI], position.y, vecteurY[cAimantI]);
      ...
      color = converge > 20 ? 255 - converge : 255;
