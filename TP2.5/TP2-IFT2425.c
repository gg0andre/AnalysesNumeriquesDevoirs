...
//Fonctions pour calculer les derivees spatielles et temporelles de l'intensite I (6)
void deriveesST(float** Img1, float** Img2, float** Ix, float** Iy, float** It, int length, int width) {
	int i,j;
...
//Fonction pour calculer les voisinnages moyennes locales de VxM et VyM (5)
void voisinML(float** VxM, float** VyM, float** Vx, float** Vy, int length, int width) {
	int i,j;
...
//Fonction pour la resolution par Jacobi (4)
void resolutionJacobi(float** OptFl_Vx, float** OptFl_Vy, float** VxM, float** VyM,
					float** Ix, float** Iy, float** It, int length, int width, float alpha) {
	...
	for (i=0; i< length; i++)
		for ( j=0; j < width; j++) {
			t = (Ix[i][j] * VxM[i][j] + Iy[i][j] * VyM[i][j] + It[i][j]) /
				(alpha*alpha + Ix[i][j]*Ix[i][j] + Iy[i][j]*Iy[i][j]);
				
			OptFl_Vx[i][j] = VxM[i][j] - Ix[i][j] * t;
			OptFl_Vy[i][j] = VyM[i][j] - Iy[i][j] * t;
		}
}
...
//	       OptFl_Vy[k][i][j]=posy; } }
 ...
 fflush(stdout);
	...
	deriveesST(Img1, Img2, Ix, Iy, It, length, width);
		
	for (k=1; k < NBITER; k++) {
		voisinML(VxM, VyM, OptFl_Vx[k-1], OptFl_Vy[k-1], length, width);
		
		resolutionJacobi(OptFl_Vx[k], OptFl_Vy[k], VxM, VyM, Ix, Iy, It, length, width, alpha);
	}
