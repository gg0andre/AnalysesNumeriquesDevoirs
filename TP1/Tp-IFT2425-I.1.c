...
/**
Fonction qui calcule f(Cmv;Yi) = 
	(Somme Yi^(Cmv)*lnYi) / (Somme Yi^(Cmv)) - 1 / Cmv - 1 / n * (Somme ln(Yi))
*/
 static inline double FonctionCmv(double cmv) {
 	double som1, som2, som3;
  		som1 = som2 = som3 = 0.0;
  	
  	for(int i=0; i<n; i++) {
  		double temp1 = pow(y[i], cmv);
  		double temp2 = log(y[i]);
	...
	return som1/som2 - 1.0/cmv -1.0/n*som3;
 }
 ...
 /**
  	...
  	f'(x) ~ (f(x+eps) - f(x))/eps 
  */
  static inline double FonctionEpsA(double cmv) {
  	return (FonctionCmv(cmv+eps) - FonctionCmv(cmv) ) / eps;
  }
  
  /**
  	Fonction epsilon 1b
  	f'(x) ~ (-f(x+2eps) + 8f(x) - 8f(x-2eps) + f(x-2eps)) / 12eps 
  */
  static inline double FonctionEpsB(double cmv) {
  	double t1 = FonctionCmv(cmv +2*eps);
  	double t2 = FonctionCmv(cmv +  eps);
  	double t3 = FonctionCmv(cmv -  eps);
  	double t4 = FonctionCmv(cmv -2*eps);
  	return (-t1 + 8*t2 - 8*t3 +t4) / (12*eps);
  }

/**
  Calculer la deriver de f(Cmv, Yi) 
  f'(Cmv, yi) = (Somme Yi^Cmv * ln(Yi)^2) / (Somme Yi^Cmv * ln(Yi)) - 1/Cmv^2
  */
  static inline double DeriveCmv(double cmv) {
  	double som1, som2;
  	som1 = som2 = 0.0;
  	
  	for(int i=0; i<n; i++) {
  		double t1 = pow(y[i], cmv);
  		double t2 = log(y[i]);
  		
  		som1 += t1 * CARRE(t2);
  		som2 += t1 * t2;
	  }
	  return som1 / som2 + 1/CARRE(cmv);
  }
  
  /**
  Calculer la forme explicite pour alpha
  alpha = (1/n * Y[i]^cmv)^(1/cmv)
  */
  double alpha(double cmv) {
	...
	return pow(1.0/n * power, 1/cmv);
  }
  
  double W(double y, double c, double a) {
...
//--------------------------------------------------------------------------------
 ...
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) Graph2D[i][j]=j/2.0;
   ...
   //implementer ici
   double cmvi, cmvf = 0.25;
   int b=1;
   do {
   	cmvi = cmvf;
   	printf("Iteration%i - Cmv = %f\t%f\n", b++, cmvf, FonctionEpsA(cmvf));
   	cmvf = cmvf - FonctionCmv(cmvf) / FonctionEpsA(cmvf);
   } while(!(fabsl(FonctionCmv(cmvf)) < tolerance || fabsl(cmvf - cmvi) < tolerance));
   ...
   for(b=0; b<width; b++) {
   	val = W(i*5.5/width, cmvf, a);
   	//printf("%.8f\n", val);
   	hauteur = val*length/1.6;
   	Graph2D[length-hauteur-1][b] = 1;
   }
