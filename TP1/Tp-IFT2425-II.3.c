...
//--------------------------------------------------------------------------------
 ...
 //implementer ici
 double cx, cj, zx, zj;
 double* x_Chemin = new double[NbIterMax];
 double* j_Chemin = new double[NbIterMax];
 
 for(double k=0.0; k<length; k+=0.1)
 	for(double l=0.0; l<width; l+=0.1) {
 		cx = 2.0 * (l - width / 1.35) / (width - 1);
 		cj = 2.0 * (k - length / 1.35) / (length - 1);
 		zx = 0.0;
 		zj = 0.0;
 		
 		for(int iter=0; iter<NbIterMax; iter++) {
 			double tx = CARRE(zx) - CARRE(zj) + cx;
 			double tj = 2 * zx * zj + cj;
 			if(CARRE(tx) + CARRE(tj) > 2.0) goto loop;
 			else {
			 zx = tx; zj = tj;
			}
			x_Chemin[iter] = zx;
		...
		for(int n=0; n<NbIterMax; n++) {
			int pointX = x_Chemin[n] * (width - 1) / 2.0 + (width/1.35);
 			int pointJ = j_Chemin[n] * (length - 1) / 2.0 + (length/ 2.0);
 			if(pointX < 0 || pointJ < 0 || pointX >= width || pointJ >= length) {
 				goto loop;	//sortir de la boucle
			}
			Graph2D[pointJ][pointX] += 1;		
		}
